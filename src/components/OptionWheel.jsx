...
const OptionWheel = ({
  ...
  // Pure layout renderer that transforms and rotates every item along the 3D curved wheel
  ...
  // Optional tick audio feedback on selection change
  ...
  // Wheel / touchpad scrolling with step accumulator for smooth rotation
  useEffect(() => {
    ...
    const onWheel = e => {
      ...
      if (Math.abs(accumulatedDelta) >= 25) {
        ...
        accumulatedDelta = 0;
        const currentSnap = Math.round(targetRef.current);
        applyTarget(currentSnap + direction, true);
      }
    };
  ...
  useEffect(() => {
    ...
    const id = requestAnimationFrame(() => renderItems(posRef.current));
    return () => cancelAnimationFrame(id);
  }, [items.length, fontSize, spacing, curve, tilt, blur, fade, minOpacity, side, loop, smoothing, applyTarget, renderItems]);
